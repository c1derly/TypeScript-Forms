namespace TSF.Utilities {
    ...
    {
        static token = /d{1,4}|m{1,4}|yy(?:yy)?|([HhMsTt])\1?|[LloSZ]|"[^"]*"|'[^']*'/g;
        ...
        public static formatDate(date:Date, format:string, utc?:boolean) {

            // You can't provide utc if you skip other args (use the "UTC:" mask prefix)
            if (arguments.length == 1 && Object.prototype.toString.call(date) == "[object String]" && !/\d/.test(<any>date)) {
                format = <any>date;
                date = undefined;
            }
            ...
            if (isNaN(<any>date)) throw SyntaxError("invalid date");
